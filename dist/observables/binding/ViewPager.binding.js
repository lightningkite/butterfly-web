...
function xViewPagerBindStatic(this_, items, showIndex = new StandardObservableProperty_1.StandardObservableProperty(0, undefined), makeView) {
    const content = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-content"));
    const left = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-left"));
    const right = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-right"));
    if (!content || !left || !right)
        return;
    let pastIndex = showIndex.value;
    DisposeCondition_ext_1.xDisposableUntil(ObservableProperty_ext_1.xObservablePropertySubscribeBy(showIndex, undefined, undefined, (it) => {
...
//! Declares com.lightningkite.butterfly.observables.binding.bind>androidx.viewpager.widget.ViewPager
function xViewPagerBind(this_, items, _default, showIndex = new StandardObservableProperty_1.StandardObservableProperty(0, undefined), makeView) {
    const content = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-content"));
    const left = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-left"));
    const right = View_ext_1.findView(this_, (e) => e.classList.contains("butterfly-pager-right"));
    if (!content || !left || !right)
    ...
    let currentObs = new StandardObservableProperty_1.StandardObservableProperty(_default);
    DisposeCondition_ext_1.xDisposableUntil(ObservableProperty_ext_1.xObservablePropertySubscribeBy(items, undefined, undefined, (newItems) => {
    ...
    DisposeCondition_ext_1.xDisposableUntil(ObservableProperty_ext_1.xObservablePropertySubscribeBy(showIndex, undefined, undefined, (it) => {
        if (it < 0 || it >= items.value.length)
            return;
        let currentObs = new StandardObservableProperty_1.StandardObservableProperty(items.value[it]);
...
//# sourceMappingURL=ViewPager.binding.js.map