...
const DateAlone_1 = require("./DateAlone");
...
function xDateSet(this_, ...things) {
    for (const item of things) {
        if (item instanceof DateAlone_1.DateAlone) {
            this_.setFullYear(item.year);
            this_.setMonth(item.month - 1);
            this_.setDate(item.day);
        }
        else if (item instanceof TimeAlone_1.TimeAlone) {
...
function xDateDateAloneGet(this_) {
    return new DateAlone_1.DateAlone(this_.getFullYear(), this_.getMonth() + 1, this_.getDate());
}
...
function xDateTimeAloneGet(this_) {
    return new TimeAlone_1.TimeAlone(this_.getHours(), this_.getMinutes(), this_.getSeconds());
}
