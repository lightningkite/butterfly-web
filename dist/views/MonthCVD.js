...
const TimeNames_1 = require("../time/TimeNames");
const StandardObservableProperty_1 = require("../observables/StandardObservableProperty");
const ObservableProperty_ext_1 = require("../observables/ObservableProperty.ext");
const Date_1 = require("../time/Date");
const Paint_1 = require("./draw/Paint");
const Colors_1 = require("./Colors");
const floorMod_1 = require("../floorMod");
const Kotlin_1 = require("../Kotlin");
const Date_2 = require("../time/Date");
const RectF_1 = require("./geometry/RectF");
const Canvas_1 = require("./draw/Canvas");
const Path_1 = require("./draw/Path");
const CustomViewDelegate_1 = require("./CustomViewDelegate");
const delay_1 = require("../delay");
//! Declares com.lightningkite.butterfly.views.MonthCVD
class MonthCVD extends CustomViewDelegate_1.CustomViewDelegate {
    constructor() {
        super();
        this.currentMonthObs = new StandardObservableProperty_1.StandardObservableProperty(Date_2.dateAloneMod(Date_1.xDateDateAloneGet(new Date()), Date.prototype.setDate, 1), undefined);
        this.dragEnabled = true;
        DisposeCondition_ext_1.xDisposableForever(ObservableProperty_ext_1.xObservablePropertySubscribeBy(this.currentMonthObs, undefined, undefined, (value) => {
        ...
        this.labelPaint = new Paint_1.Paint();
        this.dayPaint = new Paint_1.Paint();
        this.labelPaint.isAntiAlias = true;
        this.labelPaint.style = Paint_1.Paint.Style.FILL;
        this.labelPaint.color = Colors_1.numberToColor(0xFF808080);
        this.dayPaint.isAntiAlias = true;
        this.dayPaint.style = Paint_1.Paint.Style.FILL;
        this.dayPaint.color = Colors_1.numberToColor(0xFF202020);
        DisposeCondition_ext_1.xDisposableUntil(delay_1.getAnimationFrame().subscribe((timePassed) => {
        ...
        this.calcMonth = new DateAlone_1.DateAlone(1, 1, 1);
        this.calcMonthB = new DateAlone_1.DateAlone(0, 0, 0);
        this.drawDate = new DateAlone_1.DateAlone(1, 1, 1);
        this.rectForReuse = new RectF_1.RectF();
        this.rectForReuseB = new RectF_1.RectF();
        this.isTap = false;
        this.dragStartY = 0;
    }
    ...
    set currentOffset(value) {
        ...
        const temp199 = this.customView;
        if (temp199 !== null) {
            CustomView_1.customViewInvalidate(temp199);
        }
        ;
    }
    animateNextMonth() {
        Date_2.dateAloneModRelative(this.currentMonthObs.value, Date.prototype.getMonth, Date.prototype.setMonth, 1);
    ...
    dayAtPixel(x, y, existing = null) {
        ...
        const column = floorMod_1.xIntFloorMod(columnRaw, 7);
        const monthOffset = floorMod_1.xIntFloorDiv(columnRaw, 7);
        ...
        return this.dayAt(Date_2.dateAloneModRelative(DateAlone_ext_1.xDateAloneSet(this.calcMonth, this.currentMonth), Date.prototype.getMonth, Date.prototype.setMonth, monthOffset), row, column, existing !== null && existing !== void 0 ? existing : new DateAlone_1.DateAlone(0, 0, 0));
    }
    dayAt(month, row, column, existing = new DateAlone_1.DateAlone(0, 0, 0)) {
        return Date_2.dateAloneModRelative(Date_2.dateAloneMod(Date_2.dateAloneMod(DateAlone_ext_1.xDateAloneSet(existing, month), Date.prototype.setDate, 1), function (newDay) {
    ...
    draw(canvas, width, height, displayMetrics) {
        ...
        if (this.currentOffset > 0) {
            ...
            this.drawMonth(canvas, (this.currentOffset - 1) * width, width, Date_2.dateAloneModRelative(DateAlone_ext_1.xDateAloneSet(this.calcMonthB, this.currentMonth), Date.prototype.getMonth, Date.prototype.setMonth, (-1)), displayMetrics);
    ...
    onTouchDown(id, x, y, width, height) {
        ...
        const it_275 = day;
        if (it_275 !== null) {
            if (this.onTouchDownDate(it_275)) {
                return true;
            }
        }
    ...
    onTouchMove(id, x, y, width, height) {
        if (this.draggingId === id) {
            ...
            const it_287 = this.dayAtPixel(x, y, undefined);
            if (it_287 !== null) {
                return this.onTouchMoveDate(it_287);
            }
        }
        return true;
    }
    ...
    onTouchUp(id, x, y, width, height) {
        if (this.draggingId === id) {
            if (this.isTap) {
                const it_289 = this.dayAtPixel(x, y, undefined);
                if (it_289 !== null) {
                    this.onTap(it_289);
                }
            }
            ...
            const it_306 = this.dayAtPixel(x, y, undefined);
            if (it_306 !== null) {
                return this.onTouchUpDate(it_306);
            }
        }
        return true;
    }
...
class CalendarDrawing {
    ...
    day(canvas, month, date, inner, paint) {
        if (date.month === month.month && date.year === month.year) {
            Canvas_1.xCanvasDrawTextCentered(canvas, date.day.toString(), inner.centerX(), inner.centerY(), paint);
        }
        else {
            const originalColor = paint.color;
            let myPaint = paint;
            myPaint.color = Colors_1.applyAlphaToColor(paint.color, 64);
            Canvas_1.xCanvasDrawTextCentered(canvas, date.day.toString(), inner.centerX(), inner.centerY(), myPaint);
            myPaint.color = originalColor;
        }
    }
    label(canvas, dayOfWeek, inner, paint) {
        const text = TimeNames_1.TimeNames.INSTANCE.shortWeekdayName(dayOfWeek);
        Canvas_1.xCanvasDrawTextCentered(canvas, text, inner.centerX(), inner.centerY(), paint);
    }
    ...
    dayBackgroundStart(canvas, inner, outer, paint) {
        ...
        canvas.arc(inner.centerX(), inner.centerY(), Math.min(inner.width() / 2, inner.height() / 2), 0, Math.PI * 2);
        paint.complete(canvas);
        paint.render(canvas, Path_1.pathFromLTRB(outer.centerX(), inner.top, outer.right, inner.bottom));
    }
    dayBackgroundMid(canvas, inner, outer, paint) {
        paint.render(canvas, Path_1.pathFromLTRB(outer.left, inner.top, outer.right, inner.bottom));
    }
    dayBackgroundEnd(canvas, inner, outer, paint) {
        canvas.beginPath();
        canvas.arc(inner.centerX(), inner.centerY(), Math.min(inner.width() / 2, inner.height() / 2), 0, Math.PI * 2);
        paint.complete(canvas);
        paint.render(canvas, Path_1.pathFromLTRB(outer.left, inner.top, outer.centerX(), inner.bottom));
    }
}
exports.CalendarDrawing = CalendarDrawing;
CalendarDrawing.INSTANCE = new CalendarDrawing();
//# sourceMappingURL=MonthCVD.js.map