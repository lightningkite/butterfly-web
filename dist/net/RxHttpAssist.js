...
function xSingleUnsuccessfulAsError(this_) {
    return this_.pipe(operators_1.map((it) => {
        if (it.ok) {
            ...
            throw new HttpResponseError_1.HttpResponseException(it, undefined);
        }
    }));
}
...
function xSingleReadHttpException(this_) {
    return this_.pipe(operators_1.catchError((err) => {
        if (err instanceof HttpResponseError_1.HttpResponseException) {
            return rxjs_1.from(err.response.text())
                .pipe(operators_1.map((text) => {
                throw new HttpResponseError_1.HttpReadResponseException(err.response, text, err.cause);
            }));
        }
        else {
...
//# sourceMappingURL=RxHttpAssist.js.map