// Generated by Butterfly TypeScript converter - this file will be overwritten.
// File: observables/Closeable.kt
// Package: com.lightningkite.butterfly.observables

//! Declares com.lightningkite.butterfly.observables.Closeable
export interface Closeable {
    
    isDisposed(): boolean 
    
    dispose(): void 
    
    close(): void 
}
export namespace CloseableDefaults {
    export function isDisposed(this_: Closeable): boolean {
        return false;
    }
    export function dispose(this_: Closeable): void {
        this_.close();
    }
}

//! Declares com.lightningkite.butterfly.observables.Close
export class Close implements Closeable {
    public static implementsInterfaceComLightningkiteButterflyObservablesCloseable = true;
    public static implementsInterfaceIoReactivexDisposable = true;
    public readonly closer:  (() => void);
    public constructor(closer:  (() => void)) {
        this.closer = closer;
        this.disposed = false;
    }
    
    public disposed: boolean;
    
    public isDisposed(): boolean {
        return this.disposed;
    }
    
    public close(): void {
        this.disposed = true;
        this.closer();
    }
    public dispose(): void { return CloseableDefaults.dispose(this); }
}

